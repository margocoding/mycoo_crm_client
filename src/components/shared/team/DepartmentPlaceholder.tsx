...
export default function DepartmentPlaceholder() {
  ...
  return <section className="rounded-xl border border-line bg-hull/20 p-6 md:p-10">
    ...
    <p className="mt-3 max-w-xl text-sm leading-relaxed text-fog">Выберите департамент, чтобы открыть свои задачи. Показатели команды появятся позже.</p>
    ...
    <div className="mt-6 flex flex-wrap gap-3">
      {data?.departments.map((d) => <Link key={d.id} to={'/dashboard/tasks/' + d.id}
