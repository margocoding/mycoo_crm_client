...
export default function Header() {
  ...
  const handleOpenMyCoo = () => {
    if (onboardingComplete) {
      window.location.href = "/dashboard";
    } else {
      // Dispatch custom event to open registration
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            {!onboardingComplete && (
              <button
                ...
                className="hidden rounded-md border border-ion/50 bg-ion/10 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.15em] text-ion transition-all duration-300 hover:bg-ion/20 sm:inline-block"
              >
            ...
            >
              {onboardingComplete ? "Открыть MyCOO" : "Запустить MyCOO"}
            </button>
        ...
        >
          <nav className="flex flex-col gap-1 px-5 py-5">
            ...
            {/* Кнопка завершения пробного периода для мобильных */}
            {!onboardingComplete && (
              <button
                type="button"
                onClick={() => {
                ...
                className="mt-2 rounded-md border border-ion/50 bg-ion/10 px-4 py-3 text-center text-[12px] font-bold uppercase tracking-[0.15em] text-ion"
              >
                Завершить пробный период
              </button>
            )}
